...
if(req.user.id !== listing.useRef){
    return next(errorHandler(401 , 'You can only update ypur own listings'));
}
...
try {
    ...
    res.status(200).json(updateListings)
} catch (error) {
    next(error);
}
...
export const updateListings = async (req, res, next) => {
    ...
    try {
        // Correctly pass the filter as an object
...
export const getListingss = async (req, res ,next) => {
        try {
