    ...
    const handleSubmit = async (e) =>{
      ...
      try {
       ...
       if(formData.imageUrls.length < 1) return setError('You must upload atleast one image')
        if(+formData.regularPrice < +formData.discountPrice) return setError('Discount price must be lower thn regular price')
        setLoading(true);
        setError(false);
  ...
  return (
    ...
    <div className='flex flex-col gap-4 flex-1'>
      ...
      <input type='text' placeholder='Address' className='border p-3 rounded-lg' 
        id='address' required maxLength='62' minLength='10'  
...
<div className='flex flex-wrap gap-6'>
    ...
    <div className='flex items-center gap-2'>
         <input  type='number' id='regularPrice' min='1' max='100000' required 
            className='p-3 border border-gray-300 rounded-lg'   
            onChange={handleChange} 
    ...
    {formData.offer && (<div className='flex items-center gap-2'>
         <input  type='number' id='discountPrice' min='1' max='100000' required 
            className='p-3 border border-gray-300 rounded-lg' 
            onChange={handleChange} 
     ...
     {formData.imageUrls.length > 0 && formData.imageUrls.map((url , index) => (
      ...
      <div key={url} className="flex justify-between p-3 border items-center">
        ...
        <button disabled= {uploading} type='button' onClick={() => handleRemoveImage(index)}  className='p-3 text-red-700 rounded-lg uppercase hover:opacity-75'>Delete</button>
...
// promise.all is used when all the promises are executed